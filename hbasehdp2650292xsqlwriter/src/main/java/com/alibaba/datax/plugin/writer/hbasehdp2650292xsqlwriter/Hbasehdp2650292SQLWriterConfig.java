 ...
 * @author yanghan.y
 */
public class Hbasehdp2650292SQLWriterConfig {
    private final static Logger LOG = LoggerFactory.getLogger(Hbasehdp2650292SQLWriterConfig.class);
     ...
     * @return
     */
    public static Hbasehdp2650292SQLWriterConfig parse(Configuration dataxCfg) {
        assert dataxCfg != null;
        Hbasehdp2650292SQLWriterConfig cfg = new Hbasehdp2650292SQLWriterConfig();
    ...
    private static void parseClusterConfig(Hbasehdp2650292SQLWriterConfig cfg, Configuration dataxCfg) {
        // 获取hbase集群的连接信息字符串
        String hbaseCfg = dataxCfg.getString(Key.HBASE_CONFIG);
        if (StringUtils.isBlank(hbaseCfg)) {
            // 集群配置必须存在且不为空
            throw DataXException.asDataXException(
                    Hbasehdp2650292SQLWriterErrorCode.REQUIRED_VALUE,
        ...
        if (dataxCfg.getBool(Key.THIN_CLIENT, Constant.DEFAULT_USE_THIN_CLIENT)) {
            Map<String, String> thinConnectConfig = Hbasehdp2650292SQLHelper.getThinConnectConfig(hbaseCfg);
            ...
            if (Strings.isNullOrEmpty(thinConnectStr)) {
                throw DataXException.asDataXException(
                    Hbasehdp2650292SQLWriterErrorCode.ILLEGAL_VALUE,
            ...
            if (Strings.isNullOrEmpty(cfg.namespace) || Strings.isNullOrEmpty(cfg.username) || Strings
                ...
                throw DataXException.asDataXException(Hbasehdp2650292SQLWriterErrorCode.ILLEGAL_VALUE,
            ...
            try {
                zkCfg = Hbasehdp2650292SQLHelper.getHbaseConfig(hbaseCfg);
            } catch (Throwable t) {
                // 解析hbase配置错误
                throw DataXException.asDataXException(
                    Hbasehdp2650292SQLWriterErrorCode.REQUIRED_VALUE,
    ...
    private static void parsePhoenixConfig(Hbasehdp2650292SQLWriterConfig cfg, Configuration dataxCfg) {
        // 获取phoenix配置信息字符串
        String phoenixCfg = dataxCfg.getString(Key.PHOENIX_CONFIG);

        Map<String, String> configs = null;
        try {
            configs = Hbasehdp2650292SQLHelper.getPhoenixConfig(phoenixCfg);
        } catch (Throwable t) {
            // 解析phoenix配置错误
            throw DataXException.asDataXException(
                    Hbasehdp2650292SQLWriterErrorCode.ILLEGAL_VALUE,
    ...
    private static void parseTableConfig(Hbasehdp2650292SQLWriterConfig cfg, Configuration dataxCfg) {
        // 解析并检查表名
        cfg.tableName = dataxCfg.getString(Key.TABLE);
        if (cfg.tableName == null || cfg.tableName.isEmpty()) {
            throw DataXException.asDataXException(
                    Hbasehdp2650292SQLWriterErrorCode.ILLEGAL_VALUE, "HBase的tableName配置不能为空,请检查并修改配置.");
        }
        try {
        ...
        if (cfg.columns == null || cfg.columns.isEmpty()) {
            throw DataXException.asDataXException(
                    Hbasehdp2650292SQLWriterErrorCode.ILLEGAL_VALUE, "HBase的columns配置不能为空,请添加目标表的列名配置.");
        }
    }
     ...
     */
    private Hbasehdp2650292SQLWriterConfig() {
    }
}
